...
## Project ORION (Mac/Gemini Edition)
...
> **GEMINI EDITION** - Converted for Google Gemini API, December 2025
...
You might think: "Can't Gemini already do this?"
...
**Google AI Studio API:** The external AI service that provides the intelligence—both for writing code and for checking images. We use Gemini, Google's frontier model.
...
Python is readable, has extensive libraries for AI and document manipulation, and is what Google's Generative AI SDK is written in. Most AI tools are Python-first.
...
This lists all the Python libraries ORION needs. **Note the change to the Google Generative AI library:**
...
We will rewrite the verification pipeline to use **Google's Generative AI SDK**. Note how much simpler image handling is—we pass the raw bytes directly rather than creating a complex JSON structure.
...
def render_and_verify(artifact, checklist: list[str], timeout: int = 60):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Step 5: Prepare prompt and images for Gemini
        checklist_text = "\n".join(f"- {item}" for item in checklist)
        
        # We construct a list of "parts" that includes both text and image data
        parts = [f"""You are a quality assurance agent. Analyze the provided images and verify against this checklist:
...
FEEDBACK: your detailed notes here"""]
        ...
        # Read images and append them as data blobs
        ...
        try:
            ...
            # Using Gemini 3 Pro for vision capabilities
            ...
            return {"passed": False, "feedback": f"Gemini Vision error: {e}"}
EOF
```
...
This is the heart of ORION. We will rewrite the agent to use **Google's Generative AI**.

Key differences in this implementation:

1. **System Prompts:** In Gemini, the system prompt is passed once when initializing the model, not during every message call.
2. **Roles:** Anthropic uses `assistant`; Gemini uses `model`.
3. **Safety Settings:** We explicitly disable safety blocks to prevent legitimate code generation from being flagged.
...
class OrionAgent:
    ...
    def __init__(self, prompt: str):
        ...
        self.system_prompt = "" 
        # Note: We initialize the model later in run() once we have the system prompt
    ...
    def _call_llm_with_retry(self, history, system_prompt, max_retries=3):
        """Call the LLM with exponential backoff retry logic."""
        
        # Translate ORION history format to Gemini format
        ...
        # Safety settings to prevent blocking legitimate code generation
        safety_settings = {
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = model.generate_content(
                    ...
                    generation_config={"temperature": 0.0} # Lower temp for code
                )
                return response
            except Exception as e:
    ...
    def _extract_checklist(self, prompt: str):
        ...
        try:
            model = genai.GenerativeModel(
                model_name="gemini-3-pro",
                system_instruction="You are a QA expert. Generate a concise quality checklist."
            )
    ...
    def run(self):
        ...
        self.system_prompt = f"""You are an autonomous agent specialized in creating {self.skill}s.
...
4. Use the artifact classes exactly as shown in the skill guide"""
        ...
        for iteration in range(self.max_iterations):
            ...
            try:
                response = self._call_llm_with_retry(self.history, self.system_prompt)
            except Exception:
...
Docker Compose simplifies running containers. **Note the change to the API key environment variable.**
...
**IMPORTANT:** Replace `your-key-here` with your actual key from Google AI Studio.
...
- **`-e GOOGLE_API_KEY="$GOOGLE_API_KEY"`**: Pass your API key to the container.
...
- Set the key before running: `export GOOGLE_API_KEY="..."`
- Make sure you're using `-e GOOGLE_API_KEY="$GOOGLE_API_KEY"` in your docker run command
...
**Google AI Studio API:** The cloud service that provides access to Gemini, the AI model powering ORION's intelligence for code generation and vision-based quality assurance.
...
**Gemini:** Google's frontier AI model, used in ORION for both code generation and vision-based document verification.
...
## Errata & Corrections (Gemini Edition)

This Gemini Edition includes the following changes from the original OpenAI version:

### Change #1: API Library Swap
...
### Change #3: System Instruction

Gemini accepts the system prompt as an initialization parameter:
...
### Change #4: Vision Input Format

Gemini accepts raw bytes rather than Base64 JSON objects:

**Original:**
```python
"source": {"type": "base64", "data": img_str}
```
...
### Change #5: Model Names

- Code generation: `gpt-4-turbo` → `gemini-3-pro`
- Vision QA: `gpt-4o` → `gemini-3-pro`
...
*Gemini Edition prepared December 2025*
