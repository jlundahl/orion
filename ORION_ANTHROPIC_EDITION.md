...
## Project ORION (Mac/Anthropic Edition)
...
> **ANTHROPIC EDITION** - Converted for Claude API, December 2025
...
You might think: "Can't Claude already do this?"
...
**Anthropic API:** The external AI service that provides the intelligence—both for writing code and for checking images. We use Claude, Anthropic's frontier model.
...
Python is readable, has extensive libraries for AI and document manipulation, and is what Anthropic's official library is written in. Most AI tools are Python-first.
...
1. **Retry logic with exponential backoff** — If the Anthropic API fails (network issues, rate limits, server errors), the agent waits and tries again instead of crashing.
    ...
    def _extract_checklist(self, prompt: str):
        ...
        try:
            response = self.client.messages.create(
                model="claude-sonnet-4-20250514",
                max_tokens=200,
                system="You are a QA expert. Generate a concise quality checklist.",
                messages=[
                    {
                        "role": "user",
...
> **TIP:** Notice we separate the `system` prompt from `messages` in Claude's API. This is a key structural difference from other providers—Claude handles system prompts as a dedicated parameter rather than as a message role.
...
**IMPORTANT:** Replace `sk-ant-your-key-here` with your actual Anthropic API key. You can get one at console.anthropic.com.
...
- **`-e ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY"`**: Pass your API key to the container.
...
- Make sure you're using `-e ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY"` in your docker run command
...
**Anthropic API:** The cloud service that provides access to Claude, the AI model powering ORION's intelligence for code generation and vision-based quality assurance.
...
**Claude:** Anthropic's frontier AI model, used in ORION for both code generation and vision-based document verification.
...
## Errata & Corrections (Anthropic Edition)

This Anthropic Edition includes the following changes from the original OpenAI version:
...
Claude's API separates system prompts from messages:
...
- Code generation: `gpt-4-turbo` → `claude-sonnet-4-20250514`
- Vision QA: `gpt-4o` → `claude-sonnet-4-20250514`
...
*Anthropic Edition prepared December 2025*
